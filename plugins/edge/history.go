...
func init() {
	// Update the database history path following the OS for Microsoft Edge
	switch runtime.GOOS {
	case "windows":
		...
		databaseHistoryPath = path.Join(appData, "local", "Microsoft", "Edge", "User Data", "Default", "History")
	case "darwin":
		...
		databaseHistoryPath = path.Join(homeDir, "Library", "Application Support", "Microsoft Edge", "Default", "History")
	case "linux":
		// Get the home directory
		homeDir, err := os.UserHomeDir()
		if err != nil {
		...
		databaseHistoryPath = path.Join(homeDir, ".config", "microsoft-edge", "Default", "History")
	}
}
