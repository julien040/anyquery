...
- **Duplicate-call protection.** SQLite can re-scan a table during a query (joins, OR clauses). To avoid double-billing, `llm` and `image` results are memoized in memory for 5 minutes, keyed on the full input tuple. Within that window, the exact same call returns the same generation. A *submit failure* (which was never billed) is cached for only 30 s so a retry is not blocked. Restarting anyquery clears it; nothing is persisted.
...
- **Server mode timeouts.** `atlascloud_image` can block up to 90 s inside a query. Some MySQL clients default to shorter read timeouts — raise your client's read timeout if needed. If an image times out, the generation usually still completes on Atlas Cloud: the error message includes a `prediction_id` you can fetch later with `atlascloud_predictions`.
...
- **Don't pass `NULL` or `''` to an optional parameter.** Omit it instead. A `NULL`/empty-string equality constraint (e.g. `system_prompt = ''`, or `atlascloud_llm(model, prompt, NULL, 0.2)`) matches no rows and returns an empty result. To set a later optional argument while skipping an earlier one, use the `WHERE` form.
