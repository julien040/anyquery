...
func TestParquetModule(t *testing.T) {
	...
	sql.Register("sqlite3-parquet", &sqlite3.SQLiteDriver{
		ConnectHook: func(conn *sqlite3.SQLiteConn) error {
			conn.CreateModule("parquet_extract", &ParquetModule{})
			return nil
		},
	})

	db, err := sql.Open("sqlite3-parquet", ":memory:")
	require.NoError(t, err, "opening connection must not fail")

	defer db.Close()
	dbx := sqlx.NewDb(db, "sqlite3-parquet")

	_, err = db.Exec("create virtual table smallTable using parquet_extract(filepath=\"https://csvbase.com/meripaterson/stock-exchanges.parquet\")")
