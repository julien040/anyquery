...
func InsertTunnel(db *sql.DB, tunnel *tunnel) error {
	...
	_, err = db.Exec("INSERT INTO tunnels (id, hashedToken, createdAt, expiresAt, metadata, serverHost) VALUES (?, ?, ?, ?, ?, ?)",
		tunnel.ID, tunnel.AuthToken, tunnel.CreatedAt.Format(time.RFC3339), tunnel.ExpiresAt.Format(time.RFC3339), string(serializedMetadata), tunnel.ServerUrl)
	if err != nil {
		return err
	}
...
func GetTunnel(db *sql.DB, id string) (*tunnel, error) {
	row := db.QueryRow("SELECT id, hashedToken, createdAt, expiresAt, metadata, serverHost FROM tunnels WHERE id = ?", id)
	var CreatedAt, ExpiresAt string

	var tunnel tunnel
	var metadata string
	err := row.Scan(&tunnel.ID, &tunnel.AuthToken, &CreatedAt, &ExpiresAt, &metadata, &tunnel.ServerUrl)
	if err != nil {
		return nil, err
	}
