...
%type <tableName> table_name into_table_name delete_table_name select_table_name
...
delete_statement:
  with_clause_opt DELETE comment_opt ignore_opt FROM select_table_name where_expression_opt
  {
    $$ = &Delete{With: $1, Comments: Comments($3).Parsed(), Ignore: $4, TableExprs: TableExprs{&AliasedTableExpr{Expr:$6}}, Where: NewWhere(WhereClause, $7)}
  }
...
from_opt:
  %prec EMPTY_FROM_CLAUSE { // Add dual
    $$ = TableExprs{}
  }
  | from_clause
  {
...
aliased_table_name:
  select_table_name as_opt_id index_hint_list_opt
  {
...
// To avoid a shift/reduce conflict, we split a table_name declaration for two different rules:
